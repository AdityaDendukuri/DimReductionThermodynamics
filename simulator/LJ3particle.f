...
C    *******************************************************************
        ...
        REAL        RANDOM_NUMBER, DUMMY, ACATMA, PI, RATIO, SR9, SR3
	...
	REAL*8      ACM
        INTEGER     STEP,NSTEP, I, IPRINT, ISAVE, IRATIO
        LOGICAL     OVRLAP
        CHARACTER   TITLE*80, CNFILE*30, TRAJFILE*30

        PARAMETER ( PI = 3.1415927 )

C       ****************************************************************
c        INTEGER (KIND=8) :: NSTEP 
c        PRINT*, "lARGEST VALUE FOR NSTEP IS ", HUGE(NSTEP)
C    ** READ INPUT DATA **
...
c       READ (*,'(A)') TITLE
        ...
        READ (*,*) IRATIO
        WRITE(*,'('' ENTER THE INITIAL CONFIGURATION FILE NAME    '')')
        READ (*,'(A)') CNFILE
        ...
        READ (*,*) RCUT
       	WRITE(*,'('' ENTER THE TRAJECTORY FILE NAME            '')')
        READ (*,'(A)') TRAJFILE

C    ** WRITE INPUT DATA **

c        WRITE(*,'(       //1X                    ,A     )') TITLE
c        WRITE(*,'('' NUMBER OF ATOMS           '',I10   )') N
        ...
        WRITE(*,'('' CONFIG IN FILE NAME       '',A     )') CNFILE
c      WRITE(*,'('' CONFIG OUT FILE  NAME     '',A     )') CNOUTFILE
        WRITE(*,'('' DENSITY                   '',F10.4 )') DENS
        WRITE(*,'('' TEMPERATURE               '',F10.4 )') TEMP
        WRITE(*,'('' POTENTIAL CUTOFF          '',F10.4 )') RCUT
	WRITE(*,'('' TRAJECTORY FILE  NAME  '',A     )') TRAJFILE
...
C    ** WRITE OUT SOME USEFUL INFORMATION **
        ...
        WRITE(*,'('' LRC FOR             =  '',F10.4)')  VLRC
        WRITE(*,'('' LRC FOR             =  '',F10.4)')  WLRC
...
C    *******************************************************************
        ...
        OPEN ( UNIT = 100, FILE = TRAJFILE, STATUS = 'UNKNOWN')
...
C          ** MOVE I AND PICKUP THE CENTRAL IMAGE **
              ...
              RXINEW = RXIOLD + ( 2.0 * RANDOM_NUMBER ( DUMMY ) - 1.0 ) * DRMAX
              RYINEW = RYIOLD + ( 2.0 * RANDOM_NUMBER ( DUMMY ) - 1.0 ) * DRMAX
              RZINEW = RZIOLD + ( 2.0 * RANDOM_NUMBER ( DUMMY ) - 1.0 ) * DRMAX
...
C          ** CHECK FOR ACCEPTANCE **
             ...
             ELSEIF( EXP ( - DELTVB ) .GT. RANDOM_NUMBER ( DUMMY )) THEN
...
C          ** WRITE OUT RUNTIME INFORMATION **
              ...
              WRITE(*,'(F12.0,3F12.6)') ACM, RATIO, VN, PRES
...
C          ** WRITE OUT THE CONFIGURATION AT INTERVALS **
              ...
              CALL WRITCN ( TRAJFILE, STEP, SIGMA, VN )

c         WRITE(*,'('' CALLING WRITCN ON STEP   '',I10   )') STEP
...
C    ** WRITE OUT THE FINAL CONFIGURATION FROM THE RUN **
        ...
        CALL WRITCN ( TRAJFILE, NSTEP, SIGMA, VN )
...
C    ** CALCULATE FLUCTUATIONS **
        ...
        WRITE(*,'('' FLUCTUATION IN  = '',F10.6)') FLP
...
C    ** LOOP OVER ALL MOLECULES EXCEPT I  **
 ...
 100            CONTINUE
        ...
        REAL FUNCTION RANDOM_NUMBER ( DUMMY )
...
c     :         FORM = 'UNFORMATTED'                        )
        ...
        DO I = 1,N
           READ ( CNUNIT, * ) DUM, RX(I), RY(I), RZ(I)
        ...
        SUBROUTINE WRITCN ( CNFILE, MC, SIGMA, VN )

        COMMON / BLOCK1 / RX, RY, RZ

C    *******************************************************************
C    ** SUBROUTINE TO WRITE OUT THE CONFIGURATION TO UNIT 100         **
C    *******************************************************************

        INTEGER     N,I,MC
        PARAMETER ( N = 3 )
        CHARACTER   CNFILE*(*)
        REAL        RX(N), RY(N), RZ(N), SIGMA
        REAL        X(N),Y(N),Z(N), VN

        INTEGER     CNUNIT
        PARAMETER ( CNUNIT = 100 )

C   ********************************************************************

c        WRITE(*,'('' TOP OF WRITCN ON STEP   '',I10   )') MC
...
c        WRITE ( CNUNIT, * ) N
       ...
       WRITE (CNUNIT,* ) (MC,VN, X(I), Y(I), Z(I), I=1,3)
c   WRITE (CNUNIT,* ) (MC,X(I), Y(I), Z(I),VN, I=1,3)

      
c        WRITE(*,'('' BOTTOM OF WRITCN ON STEP   '',I10   )') MC
