...
#model_save_dir = 'model_save/T017_Take1.ckpt'
#model_save_dir = 'model_save/T021_Take1.ckpt'
#model_save_dir = 'model_save/T029_Take1.ckpt'
model_save_dir = 'model_save/T036_Take1.ckpt'

#Network Charecteristics
dim_H = num_particles*3   #Higher Dimension Size       
dim_layer_1 = 11          #Hidden layer 1 size
dim_layer_2 = 6           #Hidden layer 2 size       
...
#load data
features, labels = load_features_lables("../data/13P_T036_RAW.txt")
...
weights = {
    'w1': tf.Variable(glorot_init([dim_H, dim_layer_1])),
    'w2': tf.Variable(glorot_init([dim_layer_1, dim_layer_2])),
    'w3': tf.Variable(glorot_init([dim_layer_2, dim_L])),
    'w4': tf.Variable(glorot_init([dim_L, dim_output]))
}
...
optimizer_node = tf.train.AdamOptimizer(learning_rate=0.001, 
                                        beta1=0.4, 
                                        beta2=0.4, 
                                        epsilon=1e-08)
...
ax2.set_xlabel('Predicted Potential Energy')
ax2.set_ylabel('Original Potential Energy')
ax2.set_title('T = 0.36')

a = ax.scatter(red_dim_x, red_dim_y, s=5, c=labels.flatten())
c = ax2.scatter(potentials, labels)
